...
def MakeGrid():
    ...
    grid2 = [each.split() for each in grid]
    grid2 = [[int(j) for j in i] for i in grid2]
    return grid2
...
def DiagL(grid):
    ...
    for i in range(0,17):
        for j in range(3,20):
            ...
            dl3 = grid[i+2][j-2]
            dl4 = grid[i+3][j-3]
            temp = dl1*dl2*dl3*dl4
            if temp > maxim:
                maxim = temp
    return maxim
    
def Compare(rows,columns,diagr,diagl):
    maximum = max(rows,columns,diagr,diagl)
    return maximum
...
def Main():
    ...
    diagr = DiagR(grid)
    diagl = DiagL(grid)
    maximum = Compare(rows,columns,diagr,diagl)
    print(maximum)
